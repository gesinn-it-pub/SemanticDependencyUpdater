...
class SelfReferenceIntegrationTest extends SduIntegrationTestCase {
	 ...
	 * not a timestamp, is the right cycle key. Live measurement (2026-08-10)
	 * showed the fixed-backoff client can reload BEFORE the server-side
	 * self-update cycle actually finishes when a page produces several
	 * genuine (non-empty-diff) passes in a row, showing a stale intermediate
	 * value - this method is the mechanism meant to close that gap.
	 *
	 * NEVER ACTUALLY RUN in this environment as of 2026-08-10: this test
	 * class extends SduIntegrationTestCase, which unconditionally
	 * markTestSkipped()s under SMW < 7.0 (see that class's docblock) - and
	 * every container available here (both the docker-compose-ci test
	 * container and the live sandbox used for manual verification) runs SMW
	 * 5.1.0. isSelfUpdateReloadPending() was implemented and verified live
	 * instead (real browser + real save cycle against a real page, see
	 * memory's reference_reload_cycle_measurement_method), NOT via this
	 * test. Do not trust this test as passing evidence until it has
	 * actually executed (not skipped) at least once, e.g. after porting to
	 * an environment with SMW >= 7.0.
	 *
	 * @covers \SDU\Hooks::onAfterDataUpdateComplete
	 * @covers \SDU\Hooks::rebuildData
